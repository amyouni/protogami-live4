<nav class="border-b border-black/10" style="background-color: var(--brand-bg)">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        <div class="flex items-center gap-6 text-sm font-medium" style="color: var(--brand-text)">
            @php $halfNav = array_slice($navItems ?? [], 0, ceil(count($navItems ?? []) / 2)); @endphp
            @foreach ($halfNav as $item)
        ...
        <div class="flex items-center gap-6 text-sm font-medium" style="color: var(--brand-text)">
            @php $restNav = array_slice($navItems ?? [], ceil(count($navItems ?? []) / 2)); @endphp
            @foreach ($restNav as $item)
                @if (count($item['children']) > 0)
                    <div class="group relative">
                        <a href="#" data-page="{{ $item['id'] }}" class="hover:opacity-70">{{ $item['name'] }} ▾</a>
                        <div class="absolute right-0 top-full z-10 hidden min-w-36 rounded-md border border-black/10 py-1 shadow-lg group-hover:block" style="background-color: var(--brand-bg)">
                            @foreach ($item['children'] as $child)
                                <a href="#" data-page="{{ $child['id'] }}" class="block px-4 py-1.5 hover:opacity-70">{{ $child['name'] }}</a>
                            @endforeach
                        </div>
